...
public class TransferoutImprestFromExcelSheet {

	private static final int RemainingColumnIndex = 0;

	public static void main(String[] args) throws InterruptedException {

		String excelFilePath = "/home/user/git/repository/artifactid/files/LoginCred.xlsx";
		String sheetName = "TRansferoutImprest";
...
// Print the entered qty
				...
				WebElement receiveTransferBtn = wait.until(
						ExpectedConditions.elementToBeClickable(By.xpath("//p[normalize-space()='Send Transfer']")));
				...
				updateExcelWithStockAndRemaining(excelFilePath, sheetName, medicationData[0], stock, Remaining);
	...
	private static void updateExcelWithStockAndRemaining(String excelFilePath, String sheetName, String MedicationName,
			String stock, int Remaining) throws IOException {
		FileInputStream excelFile = new FileInputStream(new File(excelFilePath));
		Workbook workbook = WorkbookFactory.create(excelFile);
		Sheet sheet = workbook.getSheet(sheetName);

		int stockColumnIndex = 6;
		int RemainingColumnIndex = 7;
		...
		for (int i = 1; i <= sheet.getLastRowNum(); i++) {
			...
			if (cellValue.equals(MedicationName)) {
				...
				Cell RemainingCell = row.createCell(RemainingColumnIndex);
