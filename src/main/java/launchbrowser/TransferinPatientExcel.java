...
public class TransferinPatientExcel {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		try {
			...
			boolean medicationNotFound = true;
			for (String[] medicationData : ArrayMedication) {
			    ...
			    String patientName = medicationData[1];
			    String prescriberName = medicationData[2];

			    // Assuming quantity is in the second column (index 3)
			    String quantity = medicationData[3];

			    if (MedicationName == null || MedicationName.trim().isEmpty() || !MedicationName.equalsIgnoreCase(medicationData[0])) {
			        if (medicationNotFound) {
			            System.out.println("Medication name not available or does not match. Skipping further processing.");
			            medicationNotFound = false;  // Set the flag to false after printing the message
			        }
...
// Print the Present available balance of that Medicaiton on the Stocktake 
				...
				WebElement patientNameInput = wait.until(
				        ExpectedConditions.elementToBeClickable(By.xpath("//input[@placeholder='Patient...']")));
				patientNameInput.click();
				patientNameInput.sendKeys(medicationData[1]); // Patient name from Excel
...
// Transfer In Process
				...
				patientname.click();
				patientname.sendKeys(medicationData[1]);

				Thread.sleep(2000);
				
				WebElement ClickonsearchBTN = wait
						.until(ExpectedConditions.elementToBeClickable(By.xpath("//p[normalize-space()='Search']")));
				...
				WebElement PrescRIBERINPUT = wait
						.until(ExpectedConditions.elementToBeClickable(By.xpath("//input[@placeholder='Enter Prescriber No. or Name']")));
				PrescRIBERINPUT.sendKeys(medicationData[2]);
				
				Thread.sleep(2000);
				
				WebElement Selectprescriber = wait
						.until(ExpectedConditions.elementToBeClickable(By.xpath("/html[1]/body[1]/div[1]/div[1]/div[3]/div[1]/div[1]/div[2]/div[1]/div[1]/div[2]/form[1]/div[1]/div[2]/div[2]/div[1]/div[1]/div[1]/p[3]")));
				Selectprescriber.click();
				
				Thread.sleep(2000);
				
				
				WebElement Prescriber = wait.until(ExpectedConditions.presenceOfElementLocated(
						By.xpath("/html[1]/body[1]/div[1]/div[1]/div[3]/div[1]/div[1]/div[2]/div[1]/div[1]/div[2]/form[1]/div[1]/div[2]/div[2]/div[1]/div[1]/p[1]")));
				...
				quantityInput.sendKeys(medicationData[3]);
...
// Print the entered qty
	...
	private static void updateExcelWithStockAndSum(String excelFilePath, String sheetName, String MedicationName,
		...
		int medicationNameColumnIndex = 4;
		int stockColumnIndex = 8;
		int sumColumnIndex = 9;
		int resultColumnIndex = 10;
		...
		for (int i = 1; i <= sheet.getLastRowNum(); i++) {
			...
			if (cellValue.equals(MedicationName)) {
	            ...
	            int expectedValue = (int) row.getCell(7).getNumericCellValue();
	...
	private static ArrayList<String[]> readArrayMedicationFromExcel(String excelFilePath, String sheetName)
		...
		for (int i = 1; i <= sheet.getLastRowNum(); i++) {
		    ...
		    String[] medicationData = new String[4];

		    // Assuming medication name is in the fifth column (index 4)
		    Cell MedicationNameCell = row.getCell(4);
		    medicationData[0] = getCellValueAsString(MedicationNameCell);

		    // Assuming patient name is in the sixth column (index 5)
		    Cell patientNameCell = row.getCell(5);
		    medicationData[1] = getCellValueAsString(patientNameCell);

		    // Assuming prescriber name is in the seventh column (index 6)
		    Cell prescriberNameCell = row.getCell(6);
		    medicationData[2] = getCellValueAsString(prescriberNameCell);

		    // Assuming quantity is in the eighth column (index 7)
		    Cell quantityCell = row.getCell(7);
		    medicationData[3] = getCellValueAsString(quantityCell);
